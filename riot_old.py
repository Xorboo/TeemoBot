...
def get_user_id(nickname):
    ...
    user_content = send_request('v1.4/summoner/by-name/' + quote(nickname))
    if user_content is None:
        print('Couldn\'t find user ' + nickname)
        return -1, nickname

    user_data_json = json.loads(user_content)
    user_data = list(user_data_json.values())[0]
...
def get_user_elo(nickname):
    ...
    ranks_content = send_request('v2.5/league/by-summoner/' + user_id_str)
